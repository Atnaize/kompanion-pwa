    ...
    >
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <h3 className="truncate font-semibold text-gray-900">{effort.segment.name}</h3>
          {isNewPr && (
      ...
      <div className="shrink-0 text-center">
        <div
          className={`mx-auto flex h-9 w-9 items-center justify-center rounded-full text-lg ${
            isNewPr ? 'bg-gradient-to-br from-yellow-400 to-yellow-600 shadow' : 'bg-gray-100'
          }`}
        >
          {RANK_MEDALS[effort.pr_rank ?? 0] ?? '🎖️'}
        </div>
        {rankKey && <p className="mt-1 text-[10px] font-medium text-gray-500">{t(rankKey)}</p>}
      </div>
    </div>
  );
};
